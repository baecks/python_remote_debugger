...
def pyfunc2predef(ident, fw, identifier, py_func, is_class=True):
    ...
    try:
        ...
        if "returns" in definition:
            write_indented_lines(ident+_IDENT,fw,"return " + definition["returns"].replace(" ", "_"),False)
        else:
...
def rna2predef(BASEPATH):
    ...
    if "bpy.types" in INCLUDE_MODULES:
        from bpy import types as module
        pymodule2predef(BASEPATH, "bpy.types", module, "Types (bpy.types)")
    ...
    if "rna_info" in INCLUDE_MODULES:
        import rna_info as module
        pymodule2predef(BASEPATH, "rna_info", module, "rna_info Module internals (rna_info)")


    del module

def generate_bpy_stubs(output_path=None):
    import bpy
    if 'bpy' not in dir():
...
generate_bpy_stubs(path) #just run it! Unconditional call makes it easier to debug Blender script in Eclipse,
